  ...
  return (
    <section id="process" className="px-6 py-16">
      <Reveal className="mx-auto max-w-5xl">
        <h2 className="text-center text-2xl font-semibold text-[var(--ink)]">{process.title}</h2>
        <div className="mx-auto mt-10 grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {process.steps.map((step, index) => (
            <Reveal
              key={step.titre}
              delay={index * 100}
              className="relative rounded-lg border border-[var(--border-soft)] bg-[var(--surface-card)] p-6"
            >
              <span className="[font-family:var(--font-utility)] text-2xl font-bold text-[var(--color-orange-text)]">
                {String(index + 1).padStart(2, '0')}
              </span>
              <h3 className="mt-2 font-semibold text-[var(--ink)]">{step.titre}</h3>
